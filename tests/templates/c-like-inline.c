...
//   comment.
char *foo;    // {trigger_upper}: block comment after stuff. M4.0 // random note for future devs

// SAMPLE M4.1: Added junk before the beginning and after the end of the one-line block
//   comment.
char *foo;    /* {trigger_upper}: block comment after stuff. M4.1 */ // random note for future devs
...
// foo{trigger_upper}: This should not match. S2.0

// SAMPLE S2.1: This is an inline comment with junk in front of the trigger text.
// This isn't intended but it's a limit of the current implementation.
/* foo{trigger_upper}: This should not match. S2.1 */
...
/* {trigger_upper}: opened but not closed inline block-syntax comment S(-1)
